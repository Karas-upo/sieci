...
// 2. Pobieranie pytań i odpowiedzi (ZMIENIONO NAZWY TABEL NA pytania21 i odpowiedzi21)
$sql = "SELECT p.id as p_id, p.tresc as p_tresc, p.obrazek, p.multiple,
                o.id as o_id, o.tresc as o_tresc, o.czy_poprawna 
        FROM pytania21 p 
        LEFT JOIN odpowiedzi21 o ON p.id = o.pytanie_id
        ORDER BY p.id ASC";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($quiz as $p_id => $data) {
        // Przygotuj tablicę poprawnych ID
        $correct_ids = [];
...
<body>
    ...
    <div class="header-nav">
        ...
        <h1 style="margin:0;">Test: Warstwa Fizyczna i Łącza</h1>
    ...
    <form method="POST">
        <?php foreach ($quiz as $p_id => $q): ?>
            ...
            <div class="question-card <?php echo $card_class; ?>">
                ...
                <?php if (!empty($q['obrazek']) && $q['obrazek'] !== 'NULL'): ?>
                    <div class="image-box">
                        <img src="./img/<?php echo $q['obrazek']; ?>" alt="Obrazek do zadania">
                    </div>
                <?php endif; ?>
