...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['pytanie_id'])) {
    ...
    $rand_res = $conn->query("SELECT id FROM pytania2 ORDER BY RAND() LIMIT 1");
    if ($rand_res && $rand_res->num_rows > 0) {
        $q_id = $rand_res->fetch_assoc()['id'];
    } else {
        die("Brak pytań w bazie danych (tabela pytania2).");
    }
}
...
<head>
    <meta charset="UTF-8">
    <title>Trening (v2) - 1 pytanie</title>
    <style>
...
<body>
    ...
    <div class="nav-top">
        <a href="menu_test2.php" class="btn-back">&larr; Powrót</a>
        <h2 style="margin: 0; color: #455a64;">Trening (Baza v2)</h2>
    </div>
    ...
    <form method="POST">
        ...
        <?php else: ?>
            <a href="test2_1.php" class="btn-action btn-new">Następne pytanie &rarr;</a>
        <?php endif; ?>
    </form>
